...
router.post('/', upload.fields([{ name: 'photo', maxCount: 1 }, { name: 'fingerprint', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, message: 'Persona, arresto y foto registrados', personId });
  } catch (err) {
